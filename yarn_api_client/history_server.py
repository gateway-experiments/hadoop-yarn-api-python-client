...
class HistoryServer(BaseYarnAPI):
    ...
    """
    def __init__(self, address=None, port=19888, timeout=30, kerberos_enabled=False, https=False, https_verify=True):
        if address is None:
            self.logger.debug('Get information from hadoop conf dir')
            address, port = get_jobhistory_host_port()

        super(HistoryServer, self).__init__(address, port, timeout, kerberos_enabled, https, https_verify)
